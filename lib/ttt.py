...
class TrayModbusV2():
    def __init__(self, port='/dev/ttyS0', device=0x01, baudrate=19200, bytesize=8, parity=serial.PARITY_NONE, stopbits=1, timeout=1):
        self.modbus = [
            ...
            minimalmodbus.Instrument(port, 9),
            minimalmodbus.Instrument(port, 10),
            minimalmodbus.Instrument(port, 11),
            minimalmodbus.Instrument(port, 12),
            minimalmodbus.Instrument(port, 13),
            minimalmodbus.Instrument(port, 14),
            minimalmodbus.Instrument(port, 15),
            minimalmodbus.Instrument(port, 16),
        ]
